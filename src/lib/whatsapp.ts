 ...
 */
export async function initWhatsAppSocket(): Promise<WhatsAppServiceState> {
  if (globalThis.__wa_client_instance__ && globalThis.__wa_state__?.status === 'connected') {
    return globalThis.__wa_state__;
  }

  if (globalThis.__wa_is_initializing__) {
    return globalThis.__wa_state__!;
  }

  globalThis.__wa_is_initializing__ = true;
  ...
  try {
    ...
    const sock = makeWASocket({
      ...
      browser: ['GoChow Operations', 'Chrome', '1.0.0'],
    });
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        if (statusCode === DisconnectReason.loggedOut) {
          // Clear session files on explicit logout
          globalThis.__wa_state__ = {
            status: 'disconnected',
            qrCodeDataUrl: null,
            user: null,
          ...
          setTimeout(() => {
            initWhatsAppSocket().catch((e) => console.error('Auto reconnect error:', e));
          }, 3000);
        }
    ...
    globalThis.__wa_is_initializing__ = false;
  }
}
...
export async function startBackgroundBroadcast(
  ...
  // Run the background loop asynchronously without blocking the API response
  (async () => {
    for (let i = 0; i < validRecipients.length; i++) {
      // Check if user paused or stopped
      ...
      // Safe pacing delay between 1.8s and 2.8s to avoid anti-spam bans
