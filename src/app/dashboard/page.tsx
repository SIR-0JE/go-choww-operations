  ...
  return (
    <AppLayout>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-8 py-6 sm:py-8 space-y-6 sm:space-y-8">
        ...
        <div className="rounded-2xl bg-white border border-slate-200/90 px-5 py-3.5 text-xs flex items-center justify-between gap-3 text-slate-600 shadow-sm">
          <div className="flex items-center gap-2.5">
            <ShieldCheck className="w-4 h-4 text-emerald-600 shrink-0" />
            <span>
              <strong className="text-slate-900">Strict Settlement Rule:</strong> Metrics and charts compute verified settled orders (<code className="text-brand-700 bg-brand-50 px-1.5 py-0.5 rounded font-bold border border-brand-200">orderStatus === &quot;Completed&quot;</code> &amp; <code className="text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded font-bold border border-emerald-200">paymentStatus === &quot;success&quot;</code>) with full operational expense deductions.
            </span>
          </div>
          <span className="hidden sm:inline-block px-3 py-1 rounded-full text-[11px] font-extrabold bg-brand-50 text-brand-700 border border-brand-200 shrink-0">
      ...
      <footer className="border-t border-slate-200 bg-white py-6 px-4 text-center text-xs text-slate-500">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <span className="text-slate-600 font-medium">Sprint Goal: ₦3,500,000 Debt Liquidation by Dec 10, 2026</span>
        </div>
      </footer>
    </AppLayout>
  );
